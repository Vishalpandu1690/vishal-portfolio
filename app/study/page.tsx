...
// Which tracks are clickable
const activeTrackSlugs = ["python", "system-design"];
const trackHref: Record<string, string> = {
  python: "/study/python",
  "system-design": "/study/system-design",
};
      ...
      <div className="page-wrap section-gap">
        ...
        <section>
          ...
          <p className="prose-body" style={{ marginTop: 18, maxWidth: 520, fontSize: "1rem" }}>
            Structured learning paths with editorial-depth lessons. Each topic defines the
            problem clearly, builds the intuition, and turns it into interview-ready reasoning
            step by step.
          </p>
        </section>
        ...
        <section>
          ...
          <div className="grid-auto-subtle">
            {studyTracks.map((track) => {
              const isActive = activeTrackSlugs.includes(track.slug);
              const href = trackHref[track.slug] ?? "/study";
              return (
                <Link
                  key={track.slug}
                  href={href}
                ...
                >
                  ...
                  {isActive && (
                    <p className="mono-label" style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.18em", textTransform: "uppercase" }}>
                      Open track →
                    </p>
                  )}
                </Link>
        ...
        <section style={{ background: "var(--surface)", border: "1px solid var(--border)", borderLeft: "3px solid var(--accent)", padding: "32px 28px", maxWidth: 680 }}>
          ...
          <p className="h-display" style={{ fontSize: "clamp(20px,3vw,32px)", color: "var(--foreground)" }}>
            Every lesson starts with a clear problem, builds intuition with an example,
            then turns that intuition into code.
          </p>
        </section>
